...
def test_builtin_skills_do_not_connect_to_daytona(monkeypatch) -> None:
    ...
    monkeypatch.setattr(module, "get_daytona_backend_for_workspace", forbidden)
...
def test_persistent_store_routes_are_scoped_by_user_and_workspace() -> None:
    module.get_context_daytona_backend.cache_clear()
    backend = module.get_context_daytona_backend()
    ...
    assert module._workspace_memory_namespace(runtime) == (
        "project",
        "owner",
        "workspaces",
        "workspace",
...
def test_lazy_backend_resolves_only_on_operation(monkeypatch) -> None:
    ...
    monkeypatch.setattr(module, "get_daytona_backend_for_workspace", resolve)
    monkeypatch.setattr(
        module.ContextDaytonaSandbox,
        "_context",
        staticmethod(lambda: SimpleNamespace(workspace_id="workspace", project_id="project")),
    )
...
def test_execute_and_upload_emit_binding_and_artifact_events(monkeypatch) -> None:
    ...
    assert emitted[1]["type"] == "artifact.changed"
    assert emitted[1]["path"] == "/workspace/artifacts/report.md"

    lazy.upload_files([("/workspace/artifacts/chart.svg", b"<svg/>")])
    assert [event["type"] for event in emitted].count("sandbox.bound") == 1
    assert emitted[-1]["path"] == "/workspace/artifacts/chart.svg"
    assert emitted[-1]["checksum"] == hashlib.sha256(b"<svg/>").hexdigest()
...
def test_daytona_creation_is_private_blocked_and_binding_is_strict(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        module,
        "get_settings",
        lambda: Settings(OPENAI_API_KEY="test", DAYTONA_API_KEY="test"),
    )
    module.clear_backend_cache()

    backend = module.get_daytona_backend_for_workspace(
        workspace_id="workspace",
        project_id="project",
    )
    ...
    with pytest.raises(RuntimeError, match="refusing to create"):
        module.get_daytona_backend_for_workspace(
            workspace_id="workspace",
            project_id="project",
